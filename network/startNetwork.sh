...
configtxgen -profile PharmaOrdererGenesis -outputBlock ${PWD}/channel-artifacts/${CHANNEL_NAME}.block -channelID $CHANNEL_NAME
sleep 2
...
echo "—---------------Join transporter peer to the channel—-------------"

peer channel join -b ${PWD}/channel-artifacts/$CHANNEL_NAME.block
sleep 1
peer channel list

echo "—-------------transporter anchor peer update—-----------"
...
echo "—---------------install chaincode in transporter peer—-------------"
...
echo "—---------------Approve chaincode in transporter peer—-------------"

peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma-network.com --channelID $CHANNEL_NAME --name PharmaNetwork --version 1.0  --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 1

echo "—---------------Commit chaincode in transporter peer—-------------"
